  ...
  nh_(nh), nhp_(nhp)
{
  ...
  */
  ...
  /* cheat mode */
  sub_ground_truth_ = nh_.subscribe<geometry_msgs::PoseStamped>(ground_truth_topic_name_, 1, boost::bind(&MotionTracking::groundTruthCallback, this, _1)); //for uav groundtruth
...
void MotionTracking::stateCallback(const nav_msgs::OdometryConstPtr &state)
{
  ...
  void MotionTracking::heliportCallback(const geometry_msgs::PointStampedConstPtr &state)
  {
    ...
    transform.setOrigin( tf::Vector3(state->point.x, state->point.y, 1.1) ); // z = 1.1, hardcoding
    setHeliportPose(transform);
  }
                  ...
                  if(cheating_mode_heliport_)
                    {
                      ...
                      heliport.setOrigin(getHeliportOrigin());
                      //heliport.setOrientation(getHeliportOrientation());
              ...
              if(control_mode_ == TWIST_MODE)
                {
                  ...
                  if(cheating_mode_uav_)
                    {
                      heliport.setOrigin(getHeliportOrigin());
                      heliport.setRotation(getHeliportOrientation());
...
void MotionTracking::setHeliportPose(tf::StampedTransform transform)
{
...
tf::Quaternion MotionTracking::getHeliportOrientation()
{
  boost::mutex::scoped_lock lock(heliport_mutex_);
  return heliport_state_.getRotation();
}

tf::Vector3 MotionTracking::getHeliportOrigin()
{
  boost::mutex::scoped_lock lock(heliport_mutex_);
